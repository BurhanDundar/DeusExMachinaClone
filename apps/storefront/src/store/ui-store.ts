...
type State = {
  ...
  quantity: (id: string, size: string, value: number) => void;
};
export const useUIStore = create<State>()(
  persist(
    (set) => ({
      ...
      quantity: (id, size, value) =>
        set((s) => ({
          items: s.items.map((i) =>
            i.product.id === id && i.size === size ? { ...i, quantity: Math.max(1, value) } : i
          ),
        })),
    }),
    { name: "northline-cart", partialize: (s) => ({ items: s.items }) }
  )
);
