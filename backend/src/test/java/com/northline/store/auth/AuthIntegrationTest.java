...
class AuthIntegrationTest {
    ...
    @Test void registrationCreatesAccountAndSession() throws Exception {
        var result = register("rider@example.com").andExpect(status().isOk())
    ...
    @Test void duplicateRegistrationReturnsStableConflict() throws Exception {
        register("duplicate@example.com").andExpect(status().isOk());
        register("DUPLICATE@example.com").andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("EMAIL_ALREADY_REGISTERED"));
    }

    @Test void loginRejectsBadPasswordAndAcceptsCorrectPassword() throws Exception {
    ...
    @Test void currentUserRequiresBearerToken() throws Exception {
        ...
        mvc.perform(get("/api/users/me").header("Authorization", "Bearer " + json.get("accessToken").asText()))
                .andExpect(status().isOk()).andExpect(jsonPath("$.email").value("me@example.com"));
    }

    @Test void tokenForDeletedAccountIsRejectedAsUnauthorized() throws Exception {
        var registration = register("deleted@example.com").andReturn();
        var accessToken = objectMapper.readTree(registration.getResponse().getContentAsString())
    ...
    @Test void refreshRotatesCookieAndRestoresSessionAfterPageReload() throws Exception {
        ...
        var refreshed = mvc.perform(post("/api/auth/refresh").cookie(first))
    ...
    @Test void logoutRevokesRefreshTokenAndExpiresCookie() throws Exception {
        Cookie cookie = register("logout@example.com").andReturn().getResponse()
