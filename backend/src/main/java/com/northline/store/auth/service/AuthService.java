...
public class AuthService {
    ...
    public SessionResult register(RegisterRequest request) {
        ...
        if (users.existsByEmailIgnoreCase(email)) {
            throw new ApiException("EMAIL_ALREADY_REGISTERED", "An account already uses this email",
                    HttpStatus.CONFLICT);
        }
        var user = new User();
        user.setEmail(email);
    ...
    public SessionResult refresh(String rawToken) {
        if (rawToken == null || rawToken.isBlank()) {
            throw new ApiException("INVALID_REFRESH_TOKEN", "The session could not be refreshed",
                    HttpStatus.UNAUTHORIZED);
        }
        var stored = refreshTokens.findByTokenHash(hash(rawToken))
                .orElseThrow(() -> new ApiException("INVALID_REFRESH_TOKEN", "The session could not be refreshed",
                        HttpStatus.UNAUTHORIZED));
    ...
    private SessionResult issueSession(User user) {
        ...
        var response = new AuthResponse(accessToken, "Bearer", jwtService.expiresInSeconds(),
    ...
    public record SessionResult(AuthResponse response, String refreshToken, long refreshMaxAgeSeconds) {}
}
