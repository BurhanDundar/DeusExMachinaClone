...
public class AuthController {
    ...
    private ResponseEntity<AuthResponse> sessionResponse(AuthService.SessionResult result) {
        var cookie = ResponseCookie.from(REFRESH_COOKIE, result.refreshToken())
                .httpOnly(true).secure(secureCookie).sameSite(sameSite).path("/api/auth")
                .maxAge(Duration.ofSeconds(result.refreshMaxAgeSeconds())).build();
    ...
    private ResponseCookie clearCookie() {
        return ResponseCookie.from(REFRESH_COOKIE, "").httpOnly(true).secure(secureCookie)
                .sameSite(sameSite).path("/api/auth").maxAge(Duration.ZERO).build();
    }
}
